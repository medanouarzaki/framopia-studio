...
- **"Video-in → built comp entirely from the panel with no terminal" — NOT MET at close.** Frame analysis was reported, never driven: `npm run frames`, `npm run segment` and `npm run zones` run the Python sidecar, and the pipeline runner named them rather than running them. A video that had never been through the sidecar therefore could not be taken end to end from the panel. It no longer *silently* built a wrong comp — since session 39 the build refuses and names the command — but refusing is not driving. **Closed in Block 9 session 1: the `zones` stage drives the sidecar itself.**
