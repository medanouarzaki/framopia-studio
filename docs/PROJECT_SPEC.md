...
- **A client's own pictures never leave the machine.** `pictures` is `{ id, path, description, label? }` in his words; they appear in the picture editor beside the generated candidates, are **never sent to any model** and are **never copied into a cache**, both asserted by test.
- **A client's picture is used when a word she says is in that picture's label** (user ruling, 2026-09-04 — option A, the strict one, chosen over letting a model decide and over a picker, *so that it never surprises him and can be widened later with evidence*). A `label` is a list of words written as free text, split on anything that is not a letter or a digit, and compared with `normalizeToken` — the rule this project already uses to decide whether two words are the same word. So edge punctuation and Latin case are normalised, and **nothing else is**: no Arabic letter-form folding, no transliteration between scripts (a label saying `Botox` does not match `بوتوكس`; a client who wants both writes both), no stemming, no synonyms, no edit distance and no model call. A near-miss generates, exactly as before. The words tried are the **slot's own span**, **the naming word first** because that is the one word the model says the picture is about. When two labelled pictures both match, the **first in the client's own list** wins — there is no honest way to prefer one picture he labelled over another, and that is the only order he can see — and the plan records the word that fired so the choice is explicable. **A picture with no label is chosen by hand only**, and a client with no pictures behaves exactly as one did before any of this existed.
- **A label is written in the panel, beside the picture** (session 54). The field sits under the description on both client screens — where a client is set up and on the client card afterwards — and says what it does in words: *"Give a photo the words that mean it and it is used automatically whenever one of those words is spoken."* An empty box writes **no label at all**, which is what keeps a picture hand-chosen; changing a label afterwards is `POST /clients/picture-label` and **does not move the client's version**, because a label decides which picture answers a word next time and is not part of the look a reel pins.
- **A video can have pictures of its own** (session 54), on the picture screen, browsed and labelled exactly as a client's. They live on the plan as `plan.pictures`, numbered `own001` upward so they can never be confused with a client's `pic001`. **They are searched before the client's**, and the preference is expressed as search order rather than as a rule — `matchClientPicture` already takes the first picture whose label holds the word, so there is one matching rule and no second copy. The reel's own list wins because it is the more specific statement: a picture put on one video was chosen for that video, while a client's applies to everything they will ever make. Forgetting one **puts any slot that had chosen it back to being generated**, because a slot naming a picture nothing can resolve is a build that refuses at pre-flight.
- **Every field of a client can be corrected, and a client can be taken off the list** (session 54). `POST /clients/details` changes **only the fields that are sent**, so one control edits one thing and a blank goes on meaning *standard*; `null` is how the screen says *clear this*. **The version moves only when the look does** — the name, the faces and the subtitle baseline are in the snapshot a reel pins; the folder, the logo, the language and the shape are not. **The id never changes**, whatever the name becomes: it is the filename, the value on every plan and the key every snapshot pins. `DELETE /clients` **moves the file to `.local/deleted-clients/` rather than destroying it**, and a reel already made builds to exactly the same thing afterwards — asserted field for field, because a reel rebuilds from its pinned snapshot and `resolveClientIdentity` already treats a missing client file as *nothing to compare against*.
- **A client has three faces** (user ruling; `createClient` accepted two until session 54, so Dr Loubna Kfafi's emphasis face had to be written into her file by hand). A Latin sans for ordinary words, a Latin serif for the emphasised ones, and an Arabic, each chosen from the list After Effects itself reported — and **the chosen name is written as both the name a person reads and the `postScriptNames` entry a build uses**, because After Effects rejects any font name containing a space and the name from that list is the one it accepts.
- **The run screen is two buttons, not one** (user ruling, 2026-09-05). *Run pipeline* is the name of a mechanism and it did everything at once, so the only way to read a transcript was to buy the pictures first. It is now **Make the subtitles** and **Make the pictures**, both on screen at once with what each will cost. The second is refused until the subtitles exist and says why without a stage name in it — *"The pictures are drawn from the subtitles, so make those first."* Whether they exist is `wordsDone` from the dry run, not a price of zero: the words can be free because they are cached and still never have been written onto this plan.
- **The decision is made when slots are planned, before anything can bill.** A slot one of the client's own pictures fills is not sent to the image model at all: `slotNeedsGenerating` is the one declaration, and the cost screen quotes nothing for those slots. Deciding at build time would mean buying a square and then not using it; deciding inside the image stage would mean that stage reading a client's photographs, which it must never be able to do.
