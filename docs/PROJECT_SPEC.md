...
- **Images sit in the largest free band around the speaker's face, preferring the band above it** (user ruling, 2026-08-29). This **supersedes Block 7 session 9's top-left corner**, which is kept in the code as the comparison `npm run place:images` reports against, not deleted. The corner's own evidence still stands — it clears the face by 834–995 px on all five reels — but it is not the largest place that does: measured in `benchmarks/RESULTS-block8-image-placement.md`, the corner places 749–818 px where the band above the face holds 905–937 on eight of the corpus's nine slots, because anchoring at a corner wastes the width above the head. Realised gain **1.14x mean, 1.03–1.23x**, the spread being the one-sided jitter. The band beside the face wins on `vitasilk`'s `img002`, where the speaker's face reaches higher; a rule that only looked above would have made that slot smaller than the corner did. Two bounds hold by construction and are asserted per slot: the square never touches the face mask plus `HEAD_CLEARANCE`, and never leaves the frame.
- **`imageScale` 1.4 is not reachable and the shortfall is not the placement's.** It asks for 1076–1172 px; the largest face-clearing square anywhere on the frame is 765–937, so all nine slots are smaller than asked. Past this, size costs something a rule cannot decide: letting a picture bleed off the frame edge, spending `HEAD_CLEARANCE`, or overlapping the speaker. `imageScale` stays a client-mode value and clamps rather than overlapping anything.
