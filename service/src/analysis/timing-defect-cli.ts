...
function scribeWordsFor(videoSha: string): ScribeWord[] | null {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (words) return words.filter((w) => w.type === 'word');
  }
  return null;
}
...
pooled.sort((a, b) => a - b);
L.push(`| **pooled** | ${pooled.length} | ${f3(pooled[0] as number)} | ${f3(pooled[Math.floor(pooled.length / 2)] as number)} | ${f3(pooled[pooled.length - 1] as number)} |`);
L.push('');
...
L.push(`- The anticipation is real and large: pooled median **${f3(pooled[Math.floor(pooled.length / 2)] as number)} s**, max **${f3(pooled[pooled.length - 1] as number)} s**.`);
...
console.log(
  `zero-duration ${totalZero}, interpolated ${totalInterp}, non-monotonic ` +
    ...
    `two-word cards ${pooled.length} (median anticipation ${f3(pooled[Math.floor(pooled.length / 2)] as number)}s)`,
);
