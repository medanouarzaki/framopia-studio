# Operating memory archive — blocks 1 to 10

What `CLAUDE.md` recorded session by session, moved here **verbatim** by Block
10 session 28. Every one of these sections has a fuller counterpart in
`reports/block-N-session-M.md`; 81 of the 83 session sections were checked
against theirs by name. This file exists so that nothing was deleted, not
because it is the best copy — the per-session report is.

`git show 1c8c850:CLAUDE.md` is the file as it stood before the move.
