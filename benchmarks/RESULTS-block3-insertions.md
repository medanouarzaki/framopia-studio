...
**test-1 carries one extra row that is not an insertion.** Block 3 session 3
added a listening question for the `dial lvidéo` / `dial la vidéo` spelling,
sorted in by timestamp beside the `la` row. The user ruled that both spellings
are legal — §2 attaches the article, §5 keeps the French word's own form — so
only the audio can settle which the reference should carry. It answers
`dial lvidéo` / `dial la vidéo` rather than recovery / hallucination, and the
reference and the guide were not changed.

Generated by the existing spotcheck tool, which gained a context column, a
configurable lead-in, configurable answer labels and per-row answer overrides
for this; the timestamp
